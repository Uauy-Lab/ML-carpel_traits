...
unfiltered_data <- read.table("stigma_area.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)


#separate filename column into different columns -> example: 24485_11DAH_1-1_1106_1.jpg
...
unfiltered_data$rep <- as.character(lapply(strsplit(as.character(unfiltered_data$filename), "\\_"), "[", 3))
unfiltered_data$date <- as.character(lapply(strsplit(as.character(unfiltered_data$filename), "\\_"), "[", 4))
...
# create heading date column. example: 24485_11DAH_1-1_1106_1.jpg -> heading (GS59) date = 11/06/2020 - 11 days = 31/05/2020
...
library(plyr)
daily_mean = ddply(temp, .(Date), summarise, daily_temp = mean(Temperature_oC))
